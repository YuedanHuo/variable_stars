...
def xgb_evaluate(learning_rate, max_depth, subsample, colsample_bytree, n_estimators):
    ...
    model = FullyCompatibleXGBClassifier(
        ...
        n_estimators=n_estimators,  # Fixed for now, you can also tune this
        random_state=42
    )
...
param_bounds = {
    'learning_rate': (0.01, 0.1),   # Lower range to prevent overfitting
    'max_depth': (3, 6),             # Lower max depth to prevent overly complex trees
    'subsample': (0.7, 1.0),         # Sampling fraction to prevent overfitting
    'colsample_bytree': (0.7, 1.0),  # Feature sampling to prevent overfitting
    'n_estimators': (50, 100)        # Keep n_estimators on the lower side for smaller datasets
}
